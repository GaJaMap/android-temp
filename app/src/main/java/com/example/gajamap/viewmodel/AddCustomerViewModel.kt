...
class AddCustomerViewModel(private val tmp: String): ViewModel() {
    //LiveData
    ...
    class AddCustomerViewModelFactory(private val tmp: String)
        : ViewModelProvider.Factory {
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            // modelClass에 AddCustomerViewModel이 상속되었는지 확인
            if (modelClass.isAssignableFrom(AddCustomerViewModel::class.java)) {
                // 맞다면 MainViewModel의 파라미터 값을 넘겨줌
                return AddCustomerViewModel(tmp) as T
            }
